...
export function fetchActivities(startDate, endDate) {
  return (dispatch, getState) => {
    ...
    return apiList({ start_date: startDate, end_date: endDate }).then(
      activities => dispatch(receiveActivities(startDate, endDate, activities)),
    );
  };
}
...
export function fetchActivity(id) {
  return (dispatch, getState) => {
    ...
    return apiGet(id).then(activity => dispatch(receiveActivity(activity)));
  };
}
...
export function fetchCurrentActivity() {
  return (dispatch, getState) => {
    ...
    return apiGetCurrent().then(activity => {
...
export function updateActivity(activity) {
  return dispatch => {
    ...
    return apiSave(activity).then(activity =>
...
export function stopActivity(id, finishedAt = new Date()) {
  return (dispatch, getState) => {
    ...
    return apiSave(newActivity).then(activity => {
      dispatch(stopActivitySuccess(activity));
    });
  };
}
...
function stopCurrentActivity(finishedAt = new Date()) {
  return (dispatch, getState) => {
    return dispatch(fetchCurrentActivity()).then(() => {
      const currentActivity = Object.values(
        getState().activities.activities,
