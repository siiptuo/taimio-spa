...
export function serialize(activity) {
  return JSON.stringify({
    ...
    finished_at: activity.finished_at
      ? activity.finished_at.toISOString()
      : null,
...
export function apiList(params) {
  return getRequest(`activities?${createQueryParams(params)}`).then(data =>
    data.map(unserialize),
  );
}

export function apiGet(id) {
  return getRequest(`activities/${id}`).then(unserialize);
}

export function apiGetCurrent() {
  return apiGet('current')
    .then(activities =>
      activities.find(activity => activity.finished_at == null),
    )
    .catch(error => {
...
export function apiSave(activity) {
  ...
  if (isNew) {
    return postRequest('activities', serialize(activity)).then(unserialize);
  }
  return putRequest(`activities/${activity.id}`, serialize(activity)).then(
    unserialize,
  );
}
...
export function apiResume(activity) {
  ...
  return apiGetCurrent().then(currentActivity => {
