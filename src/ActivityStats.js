...
export function countActivitiesByHour(activities) {
  ...
  for (const activity of activities) {
    ...
    const duration =
      activity.finished_at.getTime() - activity.started_at.getTime();
      ...
      <svg width={200} height={200}>
        {paths.map((path, i) => (
          <g key={i} className="day-donut">
            <path d={path} opacity={maxHours === 0 ? 0 : hours[i] / maxHours} />
            <text
...
function getWeekDayNames() {
  ...
  return [...new Array(7)].map((_, i) =>
    formatter.format(new Date(2017, 8, 24 + i)),
  );
}
...
class DayTable extends React.Component {
  ...
  render() {
    ...
    const durations = groupActivitiesByDayOfTheWeek(this.props.activities).map(
      activities => ({
        activities,
    ...
    const columns = durations.map((d, i) => {
      ...
      const title =
        count === 0
          ? 'No activities'
          : `${count} ${count === 1 ? 'activity' : 'activities'}: ${duration(
              d.durationSum,
            )}`;
      return (
...
export class ActivityStats extends React.Component {
  ...
  componentDidMount() {
    this.props.dispatch(
      fetchActivities(date(diffDays(new Date(), -30)), date(new Date())),
    );
  }
    ...
    const activities =
      this.state.selectedTags.size === 0
        ...
        : allActivities.filter(activity =>
            Array.from(this.state.selectedTags).every(tag =>
            ...
            {tags.map(tag => (
              <tr
                ...
                className={
                  ...
                  (this.state.selectedTags.has(tag.name) ? ' selected' : '')
                }
              >
                ...
                <td className="activity-stats-duration">
                  <div
                    ...
                    style={{
                      width: `${
                        maxDuration === 0 ? 0 : tag.duration / maxDuration * 100
                      }%`,
                    }}
                  >
...
function mapStateToProps(state) {
  ...
  return {
    activities: Object.values(state.activities.activities).filter(
      activity => activity.finished_at && activity.started_at > fromDate,
    ),
    loading: false,
  };
}
